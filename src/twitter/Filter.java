...
public class Filter {
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
        ...
        // Convert all search words to lowercase for case-insensitive comparison
        ...
        for (Tweet tweet : tweets) {
            ...
            // Check if any word from the list appears in the tweet
            for (String word : lowerCaseWords) {
                // Using word boundary check to ensure matching full words only
                String regex = "\\b" + Pattern.quote(word) + "\\b";
                if (Pattern.compile(regex).matcher(tweetText).find()) {
                    result.add(tweet);
                    break; // No need to check further, move to next tweet
                }
            }
        }
